  ...
  try {
    ...
    return NextResponse.json(success({
      items: orders.map((order) => ({
        ...
        entryTime: order.entryTime ? new Date(order.entryTime).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }).replace(/:\d{2}$/, '') : null,
        exitTime: order.exitTime ? new Date(order.exitTime).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }).replace(/:\d{2}$/, '') : null,
